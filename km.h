...
class KM {
public:
    KM(int **cost, int N, int n_workers, int n_jobs)
        :cost(cost), N(N), n(n_workers), max_match(n_jobs),
         lx(new int[N]), ly(new int[N]),
         xy(new int[N]), yx(new int[N]),
         S(new bool[N]), T(new bool[N]),
         slack(new int[N]), slackx(new int[N]),
         prev(new int[N]) {
    }
private:
    int **cost, N;
    ...
    void update_labels() {
      int x, y, delta = std::numeric_limits<int>::max(); //init delta as infinity
